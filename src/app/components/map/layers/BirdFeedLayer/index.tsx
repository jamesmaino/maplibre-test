...
import { LayerComponentProps, LayerConfig, UserContext } from "../../config/layerRegistry";
...
const PucMarker = memo(({ speciesCount, baseColor, opacity = 1, upper }: any) => {
    const getColorWithLightness = ({ speciesCount, baseColor, upper }: any) => {
        ...
        return `hsl(${Math.round(h)}, ${Math.round(s)}%, ${Math.round(finalLightness)}%)`;
    };

    return (
        <svg width="40" height="40" viewBox="0 0 100 100" style={{ cursor: "pointer" }}>
            <path
                stroke="black"
                strokeWidth={5}
                strokeOpacity={opacity}
                opacity={opacity}
                fill={getColorWithLightness({ speciesCount, baseColor, upper, opacity })}
...
function BirdMarkers({ data, maxSpecies, onMarkerClick }: { data: BirdData[], maxSpecies: number, onMarkerClick: (station: BirdData) => void }) {
    return (
        <>
            {data.map((station) => {
...
function BirdFeedComponent({ data, onPopupOpen }: LayerComponentProps<BirdData[]>) {
    ...
    const handleBirdMarkerClick = (station: any) => {
        ...
        const sortedSpecies = station.speciesData?.counts?.slice().sort((a: any, b: any) => b.count - a.count) || [];
